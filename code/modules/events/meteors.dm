/datum/event/meteor_wave
	startWhen		= 35	// About one minute early warning
	endWhen 		= 37
	var/alarmWhen   = 30
	var/next_meteor = 6
	var/waves = 1
...
/datum/event/meteor_wave/tick()
	// Begin sending the alarm signals to shield diffusers so the field is already regenerated by the time actual meteors start flying around.
	...
	if(waves && activeFor >= next_meteor)
		...
		// Hull shielding QoL thing. Temporarily turns off shield diffusers to prevent the meteors from getting in through the holes created by them.
		// Doesn't apply to antag handheld variant.
		for(var/obj/machinery/shield_diffuser/SD in machines)
			if(SD.z in using_map.station_levels)
				SD.meteor_alarm(30)
...
/datum/event/meteor_wave/proc/worst_case_end()
	return activeFor + ((30 / severity) * waves) + 40
